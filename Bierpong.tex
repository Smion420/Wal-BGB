...
\par
\smallskip
\large{Im Zweifel hat immer ein Wal-WG\texttrademark-Mitglied Recht.}\label{Vorwort}
\end{center}
\restoregeometry
\pagebreak
...
Neue Features sind, dass es jetzt eine Bierpong Elo gibt, außerdem ein englisches Regelwerk.
Gedankt sei allen Leuten, die in der Wal-WG\texttrademark schon Bierpongmatches bestritten haben, und damit den Grundstein für die Entwicklung des Regelwerks gelegt haben. Außerdem geht Dank an Vinne, der die ganzen hübschen Abbildungen und das Cover für die dritte Auflage erstellt hat, sowie an generell alle die sich die Mühe gemacht haben den Spaß Korrektur zu lesen.\\\\
...
Eine Spielerin steht auf der eigenen Seite des Tisches. Es darf sich nicht irgendwo erhöht positioniert werden, außer es bringt keinen Vorteil, weil es zum Beispiel deutlich weiter hinten ist. 
...
\item Ein Team schusselt auf irgendeine Art und Weise unabsichtlich den Ball in einen eigenen Becher hinein. Dies zählt als Treffer für die Gegner, allerdings nicht für eine bestimmte Spielerin. Eine Ausnahme hierfür ist, wenn der Ball zum Team geworfen wird, damit es seine Würfe machen kann. 
...
Bei \ref{fig:spielaufbau} wird der grundlegende Aufbau eines jeden Bierpong-Spiels in der Wal-WG\texttrademark dargestellt. Als Tisch ist der hauseigene Biertisch zu verwenden und traditionell wird im Bad gespielt. Zum Werfen sind nur Tischtennis Trainings- oder Spielbälle zu verwenden. Billige, eindrückbare whacke Bierpong Tedi Bälle o.ä. sind strikt verboten. Es wird mit 0,4 - 0,5 Liter fassenden Plastikbechern gespielt. Außerdem muss für ein Spiel die Badtür ausgehängt werden. \\
In der folgenden Abbildung ist ein reguläres Spiel mit 2 Bier (je 0,5 Liter) pro Team abgebildet. Es sind auch alternative Varianten möglich, bei denen die Becher mit Wein o.ä. gefüllt werden. Theoretisch ist es auch erlaubt die Becher mit Wasser zu füllen und etwas anderes oder nichts zu trinken, dies ist aber nicht gerne gesehen. Jegliche Abweichung von einem regulären Spiel muss vom Gegnerteam bewilligt werden.
...
\begin{figure} 
    \centering
    \hspace*{-1.5cm}\includegraphics[scale = 0.27]{Grafiken/Bierpongflowneu.png}
...
Jedes Team muss sich jetzt entscheiden, ob es in diesem Spiel bei einer geraden oder bei einer ungeraden Becheranzahl stellen will. Dies wird dann mithilfe der Vorrichtung an der Wand markiert. Sollte dieser Schritt vergessen werden gilt die Markierung, die gerade an der Wand hängt.
...
\begin{figure}[h!]
     ...
     \end{subfigure}
        \caption{Beispiele für erlaubte Stellungen}
...
\begin{figure}[h!]
     ...
     \begin{subfigure}[b]{0.434\textwidth}
         ...
         \caption{Der einzelne Becher hat keinen Kontakt zur Grundlinie}
     ...
     \begin{subfigure}[b]{0.434\textwidth}
         ...
         \caption{Die 2 Bechergruppe hat keinen Kontakt zur Grundlinie}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.434\textwidth}
         \centering
         \includegraphics[width=\textwidth]{Grafiken/stellung4.png}
         \caption{Der vorderste Becher hat keinen Kontakt zur Grundlinie}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.434\textwidth}
         \centering
         \includegraphics[width=\textwidth]{Grafiken/stellung6.png}
         \caption{Auch eine kleine Lücke zu stellen ist nicht erlaubt}
     \end{subfigure}
        \caption{Beispiele für nicht erlaubte Stellungen}
        \label{fig:beispielezudennichterlaubtenStellungen}
\end{figure}
\item
    Centern: \\\\
    Wenn die Gegner nur noch einen Becher haben darf man fordern, dass die Gegner ihn auf die Center Position (siehe Abbildung \ref{fig:beispielezudenerlaubtenStellungen} und \ref{fig:beispielezudennichterlaubtenStellungen}) stellen. 
\end{enumerate}
...
Treffen beide Spielerinnen eines Teams in der selben Runde in den selben Becher, werden zusätzlich zu diesem noch zwei weitere als gehitted markiert. Welche die beiden weiteren sind, entscheidet das Team, dessen Becher es sind. \\\\
...
Dann gibt es eine Verlängerung. Das heißt, dass beide Teams ihren jeweiligen letzten Becher trinken müssen. Dann stellt jedes Team drei neue Becher in einer kleinen Pyramide auf, die eine Seite zentriert an der Grundlinie hat und befüllt sie mit der Hälfte der Menge, mit der am Anfang die zehn Becher befüllt wurden. Jetzt wird erneut ermittelt wer anfängt. (siehe Abschnitt \ref{subsub:entscheidenweranfängt})\\\\
...
Wenn ein Becher, den eine Spielerin gerade trinken muss, getroffen wird, dann hat ihr Team sofort verloren, außer:\\
Das Gegnerteam muss aus irgendwelchen Gründen keine, oder nicht alle dem Spiel zugehörigen Becher trinken (z.B weil sie Wasser in den Bechern haben und aus der Flasche trinken). Ein Wurf bei Eye to Eye in der Overtime ist hiervon ausgenommen.\\
\section{Island}\label{sec:island}
Jede Spielerin hat einmal im Spiel das Recht Island zu callen. Dazu muss sie eine beliebige Insel nennen (genaue Richtlinine, was als Insel zählt bei \ref{sub:wasistinsel}). Dies geht nur, wenn die Gegner am Beginn dieser Wurfphase noch 5 oder mehr Becher hatten. Dann muss sie einen alleinstehenden Becher bestimmen. Als alleinstehend gilt jeder Becher, in der aktuellen Aufstellung keinen Nachbarn mehr hat. Becher, die sich etwas verschoben haben zählen trotzdem noch als Nachbarn. Wenn sie ihren darauffolgenden Wurf in diesen Becher trifft, dann zählt dieser Becher als gehitted und zusätzlich noch ein weiterer. Welcher dies ist, darf das Team, das ihn trinken muss, entscheiden.\\
...
\subsection{Was ist eine Insel?}\label{sub:wasistinsel}
Als Insel zählt eine Landmasse, die komplett von Wasser umgeben sind. Diese müssen keine Länder sein, es zählt der Name der Insel. Länder, bein denen die Insel keinen gesonderten Namen hat, sind auch ok (z.B. Island). Australien zählt auch als Insel. Der Name einer Inselgruppe zählt auch (z.B. Indonesien). Ein Land, das auf einer Insel liegt, aber nicht gleichzeitig der Name ist, zählt nicht (z.B. Haiti, die korrekte Bezeichnung hierfür wäre Hispaniola).\\\\
Auch fiktive Inseln zählen. Diese müssen nicht komplett von Wasser umgeben sein, auch andere flüssige Substanzen sind ok. 
...
Diese Regel ist ähnlich wie die nackte Meile (siehe Abschnitt \ref{subsub:nacktemeile}) als keine unbedingte Pflicht zu verstehen, wenn es jemandem unangenehm ist sich auszuziehen, dann muss sie das nicht zwingend tun.\\\\
...
\begin{enumerate}[(1)]
    \item Die Berührung hat vor der eigenen kurzen Tischkante stattgefunden
    ...
    \item der weggeschlagene Ball war kein Aufsetzer
\end{enumerate}
...
\\\\
Diese Regel gilt schwächer als \ref{sub:springen}.
...
Hier werden jetzt alle Änderungen aufgeführt, die von der zweiten zur dritten Auflage entstanden sind. Angefangen mit denen, die sich durch das ganze Buch ziehen, und dann der ganze Rest von vorne nach hinten. Nicht aufgeführt sind korrigierte Kommata oder Umformulierungen, die nur der besseren Lesbarkeit dienen und an den Regeln nichts ändern.\\\\
Statt Wal-WG oder nur WG wird immer Wal-WG\texttrademark geschrieben\\
Das generische Maskulinum wurde durch das generische Femininum ersetzt\\
Es wird nun konstanter auf andere Regeln referenziert\\
Das Vorwort heißt jetzt auch tatsächlich so und nicht mehr Einleitung, außerdem wurde einiges hinzugefügt und geändert \ref{Vorwort}\\
Es wurde hinzugefügt, welche Becher als die Becher eines Teams zählen (zumindest indirekt) \ref{sec:bechertrinken}\\
Der Gastwurf wurde beim Trickshot offiziell angeführt \ref{sec:trickshot}\\
Beliebig große Teams sind jetzt erlaubt \ref{subsubsec:teameinteilung}\\
Neuer Abschnitt für vor dem Spiel hinzugefügt, in der sich entschieden wir, ob bei gerader oder ungerader Becheranzahl gestellt werden soll \ref{subsub:geradeoderungerade}\\
Die Umstellphase wurde angepasst, so dass jetzt auch bei ungerade gestellt werden kann \ref{subsub:umstellphase}\\
Die Bilder für die möglichen Stellungen wurden angepasst \ref{fig:beispielezudenerlaubtenStellungen} und \ref{fig:beispielezudennichterlaubtenStellungen}\\
Es ist jetzt aufgeschrieben, dass die onFirephase tatsächlich auch weiter geht, wenn getroffen wurde \ref{subsub:onfirephase}\\
Der Teil zum Gastspieler ist aus der Trickshotphase rausgeflogen, er ist jetzt beim Trickshot notiert\ref{subsub:Trickshotphase}\\
Bei der nackten Meile ist jetzt klar, dass ein Treffer in einen eigenen Becher oder bei Eye to Eye nicht zählt \ref{subsub:nacktemeile}\\
Es wurde eine Verständnishilfe für Besoffkis bei der Redemption hinzugefügt \ref{sec:redemption}\\
Es ist festgehalten, dass man für Island eine Insel nennen muss, mit konkreter Definition einer Insel \ref{sec:island}\\
Es ist nun klar, was beim nackten Treffen des Bitchcups passiert \ref{sec:bitchcup}\\
Die Messmethode und das Leergewicht eines Bechers wurden verzeichnet \ref{sec:80ml}\\
Der Zeitpunkt der Berührung ist beim wegschlagen ohne Aufsetzer jetzt wichtig, dies ist insbesondere relevant für den nächsten Punkt. Außerdem wurde der Fehler verbessert, dass sich die Aufsetzer Regel und die wegschlagen Regel widersprochen haben \ref{sec:ballimflugberühren}\\
Der Airball hat einen eigenen Abschnitt bekommen und ist schärfer definiert \ref{sec:Airball}\\
Die OnFire-Stacks sind besser definiert \ref{sub:onfirestacks}
Es wurde eine dritte Form von Stacks eingeführt \ref{sub:onicestacks}\\
Der Nervenberuhigungsbecher ist eingeführt \ref{sec:Nervenberuhigungsbecher}\\
Die Fälle, wenn der Ball auf den Bechern springt oder liegen bleibt sind jetzt aufgeführt \ref{sec:aufbecher}\\
...
\begin{figure}
    ...
    \caption{\\Wenn du großer WalWG\texttrademark Bierpong Anwalt werden willst kannst du hier das PDF des Regelwerks herunterladen}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[scale = 0.1]{Grafiken/Elo.png}
    \caption{\\Die Rangliste in der WalWG\texttrademark, Schriftführung ab dem 23.09.2025}
\end{figure}
\begin{figure}
    \centering
...
\begin{figure}
    ...
    \caption{\\Der Uploadlink für alle Bilder}
\end{figure}
